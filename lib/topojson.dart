...
_object(topology, o) {
  ...
  void arc(int i, List<List<num>> points) {
    ...
    final a = (arcs[i < 0 ? ~i : i] as List);
    for (var k = 0; k < a.length; k++) {
      points.add(transformPoint((a[k] as List).cast<num>(), k));
    }
  ...
  polygon(List arcs) => arcs.cast<List>().map((a) => ring(a.cast<num>())).toList();
  ...
  geometry(o) {
    ...
    switch (type) {
      ...
      case "MultiLineString":
        coordinates = (o['arcs'] as List).cast<List>().map((a) => line(a.cast<num>())).toList();
        break;
      case "Polygon":
        coordinates = polygon(o['arcs']);
        break;
      case "MultiPolygon":
        coordinates = (o['arcs'] as List).cast<List>().map(polygon).toList();
        break;
      default:
        return null;
    }
